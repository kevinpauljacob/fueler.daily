...
export default function Home() {
		...
		const fetchBlogPosts = async () => {
			try {
				const data = await axios.get(
					'https://fueler-daily.onrender.com/api/content/blog'
				);
	...
	useEffect(() => {
		const fetchDiscoverPosts = async () => {
			try {
				const data = await axios.get(
					'https://fueler-daily.onrender.com/api/content/discover'
				);
	...
	return (
		<main className={`bg-white text-black w-full ${inter.className}`}>
			<Navbar />
